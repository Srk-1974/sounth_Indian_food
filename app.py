...
# ── Final Smoke Animation Fix (SVG Based) ─────────────────────────────────────
# SVG particles are much more reliable than DIV particles in Streamlit
SMOKE_SVG = """
...
/* === SMOKE SVG ANIMATION === */
.smoke-container {{
    position:absolute; bottom:0; left:0; width:100%; height:100%; pointer-events:none; z-index:99;
}}
.smoke-svg {{ width:100%; height:100%; filter:blur(4px); }}
.smoke-svg circle {{ fill:rgba(255,255,255,0.8); opacity:0; animation: smokeRise 2.5s infinite ease-out; }}
...
/* === LOGIN PAGE === */
.login-page {{ background:linear-gradient(135deg,#FF9933 0%,#FFFFFF 50%,#138808 100%); padding:60px 0; border-radius:16px; }}
...
for i, item in enumerate(items):
    with cols[i % 4]:
        ...
        # Build layout without extra indentation to avoid Markdown code blocks
        card_html = (
        ...
        st.button(f"Add 🛒", key=f"b_{i}", use_container_width=True)
