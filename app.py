...
.login-wrapper h2 { color: #800020; font-family: 'Great Vibes', cursive; font-size: 2.4rem; margin-bottom: 4px; }
.login-wrapper .copy { font-size: 0.75rem; color: #999; margin-bottom: 24px; }
...
def delete_item(iid):
    st.session_state.items = [i for i in st.session_state.items if i["id"] != iid]
    if iid in st.session_state.cart:
        del st.session_state.cart[iid]
...
<div class="app-header">
    ...
    <p>Welcome, <strong>{st.session_state.username}</strong> &nbsp;|&nbsp; copyright © Bhadradri Technologies Inc.</p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if not st.session_state.cart:
        ...
        for iid, item in list(st.session_state.cart.items()):
            st.markdown(f"""
            <div class="cart-item">
                <span>{item['emoji']} <strong>{item['name']}</strong> × {item['qty']}</span>
                <span style="color:#FF6600;font-weight:700;">
                    {st.session_state.currency_symbol}{item['price'] * item['qty']}
                </span>
            </div>
            """, unsafe_allow_html=True)
        ...
        <div class="cart-total">
            🧾 Total: {st.session_state.currency_symbol} {cart_total()}
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.button("✅ Checkout", use_container_width=True, type="primary"):
            st.balloons()
            st.success(f"🎉 Order placed successfully! Total: {st.session_state.currency_symbol}{cart_total()}")
...
filtered = [
    ...
    and (selected_cat == "All" or i["category"] == selected_cat)
]
...
if not filtered:
    ...
    for row in rows:
        ...
        for col, item in zip(cols, row):
            with col:
                hot_badge = "🔥" if item.get("hot") else "❄️"
                st.markdown(f"""
                <div class="food-card">
                    ...
                    <p class="price">{st.session_state.currency_symbol} {item['price']}</p>
                    <p style="font-size:0.8rem;color:#999;">{hot_badge} {'Hot & Fresh' if item.get('hot') else 'Chilled'}</p>
                </div>
                """, unsafe_allow_html=True)
