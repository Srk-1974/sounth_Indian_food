...
.food-card:hover {{ transform:translateY(-4px); border-color:#FF5500; box-shadow:0 15px 35px rgba(255,153,51,0.2); }}
...
.chat-tooltip {{
    ...
    border: 1px solid #ddd; animation: fadeIn 0.5s ease-out;
}}
.chat-tooltip::after {{
...
/* Chat UI Styling */
.chat-header-bar {{
    background: #f8f9fa; padding: 15px; border-radius: 20px 20px 0 0;
    border: 2px solid #FF9933; border-bottom: none;
    display: flex; align-items: center; gap: 15px;
}}
.chat-header-title {{ font-weight: 700; color: #800020; font-size: 1.3rem; margin: 0; }}
.chat-container-inner {{
    background: white; border: 2px solid #FF9933; border-radius: 0 0 20px 20px;
...
with st.sidebar:
    ...
    if not st.session_state["cart"]: st.info("Empty Cart")
        ...
        for iid, it in st.session_state["cart"].items():
            sub = it['price']*it['qty']; cart_tot += sub; st.write(f"**{it['name']}** x {it['qty']} - ₹{sub}")
        st.markdown(f"### Pay: ₹{cart_tot}")
...
def get_bot_reply(msg):
    ...
    if any(k in m for k in ["hi", "hello", "hey"]): return "Hello! 👋 I'm **Foodie Bot**, your personal chef! I can help you with the menu, prices, and recommendations. What are you craving today?"
    if any(k in m for k in ["breakfast", "morning"]): 
        items = [i["name"] for i in menu if i["category"]=="Breakfast"]
        return f"🌅 Morning! Our breakfast specials are: {', '.join(items)}. Soft Idly or Crispy Dosa? Choose one! 😊"
    if any(k in m for k in ["price", "cost", "how much"]): return "💰 Our food is very affordable! Prices start at ₹10 and go up to ₹50."
    if any(k in m for k in ["best", "popular", "recommend"]): return "🌟 If you want the REAL taste, try the **DOSA** or **POORI**! They are our bestsellers."
    if any(k in m for k in ["spicy", "hot"]): return "🌶️ Try our snacks like **MASALA VADA** for a perfect spicy punch!"
    return "I'm here to help! 🍛 Ask me about breakfast, snacks, or recommendations."

# Visual Float (Avatar + Tooltip)
hr = datetime.now().hour
txt_grt = "Good Morning" if hr < 12 else "Good Afternoon" if hr < 18 else "Good Evening"
...
st.markdown("---")
# Header with Avatar
av_h = f'<img src="{CHEF_B64}" style="width:50px;height:50px;border-radius:50%;border:2px solid #FF9933;">' if CHEF_B64 else "🤖"
st.markdown(f'<div class="chat-header-bar">{av_h}<h3 class="chat-header-title">Chat with Foodie Bot</h3></div>', unsafe_allow_html=True)
...
with chat_area:
    for ms in st.session_state["messages"]:
        # Use Chef avatar for bot messages
        av = CHEF_B64 if ms["role"] == "assistant" else None
        with st.chat_message(ms["role"], avatar=av): st.markdown(ms["content"])

# Quick Replies
qc1, qc2, qc3, qc4 = st.columns(4)
with qc1:
    if st.button("🌅 Breakfast"): st.session_state["messages"].append({"role":"user","content":"Breakfast items?"}); st.session_state["messages"].append({"role":"assistant","content":get_bot_reply("breakfast")}); st.rerun()
with qc2:
    if st.button("🌶️ Spicy"): st.session_state["messages"].append({"role":"user","content":"Is it spicy?"}); st.session_state["messages"].append({"role":"assistant","content":get_bot_reply("spicy")}); st.rerun()
with qc3:
    if st.button("🌟 Best Seller"): st.session_state["messages"].append({"role":"user","content":"What's best?"}); st.session_state["messages"].append({"role":"assistant","content":get_bot_reply("best")}); st.rerun()
with qc4:
    if st.button("💰 Prices"): st.session_state["messages"].append({"role":"user","content":"Show prices"}); st.session_state["messages"].append({"role":"assistant","content":get_bot_reply("price")}); st.rerun()

if p := st.chat_input("Ask Foodie Bot a question..."):
    st.session_state["messages"].append({"role": "user", "content": p})
    with chat_area.chat_message("user"): st.markdown(p)
    
    reply = get_bot_reply(p)
    st.session_state["messages"].append({"role": "assistant", "content": reply})
    with chat_area.chat_message("assistant", avatar=CHEF_B64): st.markdown(reply)
    st.rerun()
st.markdown('</div>', unsafe_allow_html=True)
